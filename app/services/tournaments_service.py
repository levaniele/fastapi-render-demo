...
# get_tournament_by_id(db, tournament_id)      - Get tournament details
...
from app.core.cache import cached, invalidate_tournament_cache, invalidate_by_pattern
...
def upsert_tournament_winners(
    ...
    try:
        ...
        # Invalidate tournament winners cache
        tournament = db.query(Tournament).filter(Tournament.id == tournament_id).first()
        if tournament:
            invalidate_by_pattern(f"get_tournament_winners:{tournament.slug}")
            invalidate_by_pattern("get_tournament_winners")
...
def get_tournament_stats(db: Session, slug: str):
    ...
    try:
        ...
        logger.error(f"Error fetching tournament stats: {e}", exc_info=True)
        raise
    ...
    try:
        ...
        for tie in match_ties:
            ...
            for match in individual_matches:
                ...
                # For doubles, get both players from tournament_lineups
                if match["match_type"] == "doubles":
                    # Get doubles pairs from tournament_lineups for both clubs
                    r_doubles = db.execute(
                        text(
                            """
                            SELECT
                                tl.club_id,
                            ...
                            JOIN tournament_groups tg ON tl.tournament_id = tg.tournament_id
                            JOIN players p1 ON tl.player_id = p1.id
                            LEFT JOIN players p2 ON tl.player_2_id = p2.id
                            WHERE tg.id = :group_id
                                AND tl.category = :category
                                AND tl.club_id IN (:club_1_id, :club_2_id)
                            """
                        ),
                        {
                            "group_id": tie["group_id"],
                            "category": match["category"],
                            "club_1_id": tie["club_1_id"],
                            "club_2_id": tie["club_2_id"],
                        },
                    )
                    doubles_rows = {r["club_id"]: r for r in r_doubles.mappings().all()}

                    # Format as "Player1 / Player2" for each club
                    club1_lineup = doubles_rows.get(tie["club_1_id"])
                    club2_lineup = doubles_rows.get(tie["club_2_id"])

                    if club1_lineup:
                        p1_name = club1_lineup["player1_name"] or ""
                        p2_name = club1_lineup["player2_name"] or ""
                        player1 = f"{p1_name} / {p2_name}" if p2_name else p1_name
                    else:
                        player1 = "TBD"

                    if club2_lineup:
                        p1_name = club2_lineup["player1_name"] or ""
                        p2_name = club2_lineup["player2_name"] or ""
                        player2 = f"{p1_name} / {p2_name}" if p2_name else p1_name
                    else:
                        player2 = "TBD"
                else:
                ...
                transformed_matches.append(
                    {
                        ...
                        "winner_club_id": match.get("winner_club_id"),
                    }
                )
...
class TournamentService:
    ...
    def create_tournament(db: Session, tournament_data: TournamentCreate) -> Tournament:
        ...
        try:
            ...
            invalidate_by_pattern("get_all_tournaments")
