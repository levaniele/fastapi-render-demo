...
# GET  /tournaments/{tournament_id}                 - Tournament details
# GET  /tournaments/{tournament_id}/stats           - Tournament statistics
# GET  /tournaments/{tournament_id}/matches         - Tournament matches
# GET  /tournaments/{tournament_id}/standings       - Tournament standings
# GET  /tournaments/{tournament_id}/teams           - Tournament teams/roster
# GET  /tournaments/{tournament_id}/players         - Tournament players
# GET  /tournaments/{tournament_id}/staff           - Tournament staff
# GET  /tournaments/{tournament_id}/matches/{match_id}/rallies - Match rallies
...
def _resolve_tournament_slug(db: Session, tournament_id: int) -> str:
    tournament = tournaments_service.get_tournament_by_id(db, tournament_id)
    if not tournament:
        raise HTTPException(
...
def create_tournament_winners(
    payload: TournamentWinnersCreate,
...
def update_tournament_winners(
    tournament_id: int,
    payload: TournamentWinnersUpdate,
    db: Session = Depends(get_db_session),
    current_user: dict = Depends(require_role("admin")),
):
...
def get_tournament_by_id(tournament_id: int, db: Session = Depends(get_db_session)):
    """Fetch basic tournament information by id."""
    try:
        tournament = tournaments_service.get_tournament_by_id(db, tournament_id)
...
def get_tournament_stats(tournament_id: int, db: Session = Depends(get_db_session)):
    """Fetch comprehensive tournament statistics."""
    try:
        slug = _resolve_tournament_slug(db, tournament_id)
...
def get_tournament_matches(tournament_id: int, db: Session = Depends(get_db_session)):
...
def get_tournament_standings(
    tournament_id: int, group_name: Optional[str] = None, db: Session = Depends(get_db_session)
):
    """
...
@router.get("/{tournament_id}/teams", response_model=List[TeamRoster])
def get_tournament_teams(tournament_id: int, db: Session = Depends(get_db_session)):
    """
...
def get_tournament_players(tournament_id: int, db: Session = Depends(get_db_session)):
...
@router.get("/{tournament_id}/staff")
def get_tournament_staff(tournament_id: int, db: Session = Depends(get_db_session)):
    """
...
@router.get("/{tournament_id}/matches/{match_id}/rallies")
def get_match_rallies(tournament_id: int, match_id: int, db: Session = Depends(get_db_session)):
    """
    Fetch rallies for a match.
    Matches URL: /tournaments/{tournament_id}/matches/{match_id}/rallies
    """
    try:
        _resolve_tournament_slug(db, tournament_id)
