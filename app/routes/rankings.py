...
def get_global_rankings(
    ...
    per_category_limit: Optional[int] = Query(
        None,
        ge=1,
        le=200,
        description="When category is omitted, return top N per category in a single call",
    ),
    ...
    try:
        ...
        if category:
            ...
            query = text(
                ...
                WITH ranked AS (
                    SELECT
                        ...
                        ROW_NUMBER() OVER (
                            ...
                            ORDER BY pr.current_rank ASC NULLS LAST, pr.total_points DESC, pr.player_id ASC
                        ) as rn
                    FROM player_rankings pr
                    JOIN players p ON pr.player_id = p.id
                    LEFT JOIN clubs c ON p.club_id = c.id
                    WHERE pr.category IN ('MS', 'WS', 'MD', 'WD', 'XD')
                )
                SELECT
                    player_id,
                    category,
                ...
                ORDER BY category, rank NULLS LAST, points DESC
                """
            )
            params = {"per_category_limit": per_category_limit}
        else:
            query = text(
                """
                SELECT 
